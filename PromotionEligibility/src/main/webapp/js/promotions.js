...
app.controller("displayPromotionCtrl", function($scope, $http) {
	...
	$scope.selectPromotion = function(x) {
		...
		$scope.clickedPromotions.startDate = new Date(
				$scope.clickedPromotions.startDate);
		$scope.clickedPromotions.endDate = new Date(
				$scope.clickedPromotions.endDate);
	}
...
app.controller("promotionCtrl", function($scope, $http, $window) {
	$scope.statenames = [ "Select states", "Florida", "Georgia", "New Jersey",
			"California", "Texas", "Maryland", "louisana", "Washington",
			"Massachusetts", "Puerto Rico" ];
